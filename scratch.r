...
progressDisease <- function(dt) {
  ...
  ## CD4 Efflux
  dt[!is.na(cd4_duration), diff := diff - count * exp(1 - 1/cd4_duration)]
  
  ## VL Efflux
  dt[!is.na(vl_duration), diff := diff - count * exp(1 - 1/vl_duration)]
  ...
  prev_cd4[, cd4_influx := count * exp(1 - 1/cd4_duration)]
  prev_cd4[, cd4 := cd4 + 1]
  ...
  prev_vl[, vl_influx := count * exp(1 - 1/vl_duration)]
  prev_vl[, vl := vl + 1]
